...
public class ConfigEditor
{
    ...
    private static readonly ConfigItem[] Items =
    {
        ...
        new("githubToken", "GitHub API token for PRs", ConfigItemType.String),
        new("gitlabToken", "GitLab API token for MRs", ConfigItemType.String),
    };
    ...
    private string GetDisplayValue(ConfigItem item)
    {
        return item.Name switch
        {
            ...
            "githubToken" => string.IsNullOrEmpty(_config.GithubToken) ? "(none)" : "***",
            "gitlabToken" => string.IsNullOrEmpty(_config.GitlabToken) ? "(none)" : "***",
            _ => ""
        };
    }
    ...
    private void ApplyEdit(ConfigItem item, string value)
    {
        switch (item.Name)
        {
            ...
            case "maxStagedLines":
                ...
                break;
            case "githubToken":
                _config.GithubToken = string.IsNullOrEmpty(value) ? null : value;
                break;
            case "gitlabToken":
                _config.GitlabToken = string.IsNullOrEmpty(value) ? null : value;
                break;
        }
    }
}
